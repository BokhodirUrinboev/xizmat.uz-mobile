...
class _RegistrattionPageState extends State<RegistrattionPageWidget> {
  ...
  continued() {
    _currentStep < 1 ? setState(() => _currentStep += 1) : {
  ...
  void send() async {
    var res = await client.mutation(document: CREATE_SERVICE, variables: {
      "formData": fd.FormData(
          address_info: basicKey.currentState!.getAdressInfo(),
          biographic_data:
          familyContentStatusKey.currentState!.getBiographicdata(),
          passport_data:
          familyContentStatusKey.currentState!.getPassportData(),
    ...
    if (res != null) {
      ...
      showComplateSnackBar(S.current.online_reg, "Ваша заявка принята");
    }
  }
